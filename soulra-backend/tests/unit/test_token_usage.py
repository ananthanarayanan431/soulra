...
async def test_persist_token_usage_writes_log_and_increments_user(test_db, test_user):
    ...
    await persist_token_usage(
        test_db, user_id=test_user.id, conversation_id=None, usage_metadata=usage_metadata
    )

    logs = (
        (await test_db.execute(select(TokenUsageLog).where(TokenUsageLog.user_id == test_user.id)))
        .scalars()
        .all()
    )
    assert len(logs) == 2
...
async def test_persist_token_usage_noop_for_empty_metadata(test_db, test_user):
    await persist_token_usage(
        test_db, user_id=test_user.id, conversation_id=None, usage_metadata={}
    )
